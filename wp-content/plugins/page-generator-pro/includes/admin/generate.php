...
class Page_Generator_Pro_Generate {
	...
	public function generate_content( $group_id, $index = 0, $test_mode = false, $system = 'browser', $last_generated_post_date_time = false ) {
		...
		if ( isset( $settings['apply_synonyms'] ) && $settings['apply_synonyms'] ) {
			$result = $this->base->get_class( 'spintax' )->add_spintax( $settings['content'] );

			// If an error occured, don't modify the Group Content.
			if ( is_wp_error( $result ) ) {
				// Bail if in Test Mode and an error occured.
				...
				// Assign spintax to Content.
				$settings['content'] = $result;
			}
		}
		...
		switch ( $settings['overwrite'] ) {
			...
			case 'overwrite_preseve_date':
				...
				$settings = $this->remove_content_from_settings( $settings );
				break;
	...
	public function generate_term( $group_id, $index, $test_mode = false, $system = 'browser' ) {
		...
		if ( isset( $settings['apply_synonyms'] ) && $settings['apply_synonyms'] ) {
			$result = $this->base->get_class( 'spintax' )->add_spintax( $settings['excerpt'] );

			// If an error occured, don't modify the Excerpt.
			if ( is_wp_error( $result ) ) {
				// Bail if in Test Mode and an error occured.
				if ( defined( 'PAGE_GENERATOR_PRO_DEBUG' ) && PAGE_GENERATOR_PRO_DEBUG ) {
					return $this->generate_error_return(
				...
				// Assign spintax to Excerpt.
				$settings['excerpt'] = $result;
			}
		}
	...
	private function build_search_replace_arrays_for_keyword( $keyword_with_modifiers, $keyword, $term ) {
		...
		if ( $keyword_with_modifiers == $keyword ) { // phpcs:ignore
		...
		if ( $keyword_column_start_bracket_position !== false && $keyword_column_end_bracket_position !== false ) {
			...
			$column_index = array_search( $column, $this->keywords['columns'][ $keyword ] ); // phpcs:ignore
	...
	private function apply_nearby_keyword_transformation( $term, $keyword, $column, $latitude_column, $longitude_column, $radius = 3, $order_by_distance = false ) {
		...
		foreach ( $this->keywords['terms'][ $keyword ] as $row => $keyword_term ) {
			// Split the term.
			$term_parts = str_getcsv( $keyword_term, $this->keywords['delimiters'][ $keyword ] );
			...
			if ( trim( $term_parts[ $column_index ] ) != $term ) { // phpcs:ignore
				continue;
			}

			// If here, we found the current Term in the Keyword data.
			// Fetch the column indicies for latitude and longitude.
			$column_index_latitude  = array_search( $latitude_column, $this->keywords['columns'][ $keyword ] ); // phpcs:ignore
			$column_index_longitude = array_search( $longitude_column, $this->keywords['columns'][ $keyword ] ); // phpcs:ignore
			...
			$term_latitude  = trim( $term_parts[ $column_index_latitude ] );
			$term_longitude = trim( $term_parts[ $column_index_longitude ] );
			break;
		}
		...
		foreach ( $this->keywords['terms'][ $keyword ] as $row => $keyword_term ) {
			...
			$latitude   = trim( $term_parts[ $column_index_latitude ] );
			$longitude  = trim( $term_parts[ $column_index_longitude ] );

			// Skip if value is the Term.
			if ( $value == $term ) { // phpcs:ignore
				continue;
			}
	...
	private function remove_content_from_settings( $settings ) {
		...
		// Determine if we are overwriting the content.
		$overwrite = ( ! array_key_exists( 'post_content', $settings['overwrite_sections'] ) ? false : true );

		// If overwriting is enabled, no need to exclude the content.
		if ( $overwrite ) {
			return $settings;
		}
	...
	private function featured_image( $post_id, $group_id, $index, $settings, $post_args ) {
		...
		switch ( $settings['featured_image_source'] ) {
			...
			case 'creative_commons':
				...
				if ( empty( $settings['featured_image'] ) ) {
					return new WP_Error( 'page_generator_pro_generate', __( 'Featured Image: Creative Commons: No Term was specified.', 'page-generator-pro' ) );
				}
			...
			case 'pexels':
				// Bail if no Featured Image Term specified.
				if ( empty( $settings['featured_image'] ) ) {
					return new WP_Error( 'page_generator_pro_generate_featured_image', __( 'Featured Image: Pexels: No Term was specified.', 'page-generator-pro' ) );
				}
				...
				// Run query to fetch total number of pages of results that are available.
				$page_count = $this->base->get_class( 'pexels' )->page_count( $settings['featured_image'], 'original', $settings['featured_image_orientation'], $per_page );
				...
				// Run images query.
				$images = $this->base->get_class( 'pexels' )->photos_search( $settings['featured_image'], 'original', $settings['featured_image_orientation'], $per_page, $page_index );
			...
			case 'pixabay':
				// Bail if no Featured Image Term specified.
				if ( empty( $settings['featured_image'] ) ) {
					return new WP_Error( 'page_generator_pro_generate', __( 'Featured Image: Pixabay: No Term was specified.', 'page-generator-pro' ) );
				}
			...
			case 'wikipedia':
				// Bail if no Featured Image Term specified.
				if ( empty( $settings['featured_image'] ) ) {
					return new WP_Error( 'page_generator_pro_generate', __( 'Featured Image: Wikipedia: No Term was specified.', 'page-generator-pro' ) );
				}

				// Run images query.
				$images = $this->base->get_class( 'wikipedia' )->get_images( $settings['featured_image'] );
	 ...
	 * @param   int   $group_id           Group ID.
	 * @param   array $settings           Group Settings.
	 * @return  mixed                       WP_Error | Parent ID
	 */
	private function get_post_parent( $group_id, $settings ) {
		...
		if ( ! $settings['pageParent'][ $settings['type'] ] ) {
			return 0;
		}
		...
		// If the Post Parent is a number, check it exists.
		if ( is_numeric( $post_parent ) ) {
		...
		if ( ! $this->base->get_class( 'groups' )->is_generated_by_group( $post_parent, $group_parent_id ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Parent page with %1$s %2$s exists, but the parent page was not generated by the specified Parent Group ID %3$s. Either remove the Parent Group setting, or generate the Parent Page through the Parent Group first.', 'page-generator-pro' ),
					is_numeric( $post_parent ) ? __( 'ID', 'page-generator-pro' ) : __( 'Slug', 'page-generator-pro' ),
					$post_parent,
					$group_id
				)
			);
	 ...
	 * @param   array $settings   Group Settings.
	 * @param   array $post_args  wp_insert_post() / wp_update_post() arguments.
	 * @return  mixed               false | Menu Item ID
	 */
	 ...
	 * @param   array $settings   Group Settings.
	 * @param   array $post_args  wp_insert_post() / wp_update_post() arguments.
	 * @return  mixed               false: No Terms assigned due to Group Settings.
	 *                              true: Terms assigned.
	 ...
	 */
	private function generate_comments( $post_id, $group_id, $index, $settings, $post_args ) { // phpcs:ignore
		...
		for ( $comments_index = 0; $comments_index < $limit; $comments_index++ ) {
			...
			if ( count( $this->keywords['terms'] ) ) {
				$this->build_search_replace_arrays(
					...
					$this->get_keywords_terms( $settings['method'], $comments_index )
				);
			}
	 ...
	 * @param   array $settings         Group Settings.
	 * @return  mixed                   WP_Error | Comment ID
	 */
	private function generate_comment( $post_id, $group_id, $comments_index, $settings ) { // phpcs:ignore
	...
	private function get_term_path_id( $taxonomy, $term_path ) {
		...
		foreach ( explode( '/', $term_path ) as $term ) {
			...
			if ( is_null( $existing_terms->terms ) || ! count( $existing_terms->terms ) ) {
	...
	public function delete_comments( $group_id, $post_id ) {
		 ...
		 * @param   int     $group_id       Group ID.
		 * @param   int     $post_id        Generated Post ID.
		 * @param   int     $comment_ids    Generated Comment IDs that were deleted.
		 */
	 ...
	 * @param   mixed $exclude_term_ids   Exclude Term IDs.
	 * @return  mixed                       WP_Error | array
	 */
	public function get_generated_term_ids( $group_id, $limit = 999999, $exclude_term_ids = false ) {
		...
		if ( ! $terms->terms || count( $terms->terms ) === 0 ) {
			return new WP_Error(
	...
	public function get_generated_comment_ids( $group_id, $post_id ) {
		...
		if ( ! $comments->comments || count( $comments->comments ) === 0 ) {
			return false;
		}
	...
	/**
	 * Gets featured image IDs for the given Post IDs
	 *
	 * @since   2.7.9
	 *
	 * @param   array $post_ids   Post IDs.
	...
	/**
	 * Deletes Featured Images assigned to the given Post IDs and Group ID
	 *
	 * @since   2.7.9
	 *
	 * @param   array $post_ids   Post IDs.
	 ...
	 * @param   int   $group_id           Group ID.
	 * @param   int   $limit              Number of Generated Posts to delete (-1 = all).
	 * @param   mixed $exclude_term_ids   Exclude Term IDs from deletion.
	 * @return  mixed                       WP_Error | Success
	 */
	...
	/**
	 * Returns the given date/time string, offset based on WordPress' timezone settings.
	 *
	 * @since   3.1.6
	 *
	 * @param   string $date_time  Date and Time (yyyy-mm-dd hh:ii:ss).
	 * @return  string              Date and Time, offset based on WordPress' timezone settings
	 */
	private function offset_date_time( $date_time ) {

		// Get GMT offset from WordPress settings.
		$gmt_offset = $this->get_gmt_offset();

		// If there is no offset, we don't need to do anything else.
		if ( ! $gmt_offset ) {
			return $date_time;
		}
	 ...
	 */
	private function get_gmt_offset() {

		// Get GMT offset.
		$gmt_offset = get_option( 'gmt_offset' );

		// Don't do anything if the offset is zero.
		if ( $gmt_offset == 0 ) { // phpcs:ignore
			return '+0000';
		}
