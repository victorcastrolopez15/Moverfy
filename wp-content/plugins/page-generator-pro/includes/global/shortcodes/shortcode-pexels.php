 ...
 * @version 2.5.1
 */
class Page_Generator_Pro_Shortcode_Pexels extends Page_Generator_Pro_Shortcode_Base {
	...
	public function get_overview() {
		...
		return array(
			'title'                       => __( 'Pexels', 'page-generator-pro' ),
			'description'                 => __( 'Displays an image from Pexels, based on the given search parameters.', 'page-generator-pro' ),
			'icon'                        => $this->base->plugin->url . 'assets/images/icons/pexels.svg',
			'category'                    => $this->base->plugin->name,
			'keywords'                    => array(
				__( 'Pexels', 'page-generator-pro' ),
			...
			'gutenberg_icon'              => file_get_contents( $this->base->plugin->folder . '/assets/images/icons/pexels.svg' ), /* phpcs:ignore */
		);
		...
		return array(
			...
			'copy'             => array(
				...
				'data'          => array(
					...
					'container' => '.components-panel, .pexels',
				),
	...
	public function render( $atts ) {
		...
		$page_count = $this->base->get_class( 'pexels' )->page_count( $atts['term'], 'original', $atts['orientation'], $per_page );
		...
		// Run images query.
		$images = $this->base->get_class( 'pexels' )->photos_search( $atts['term'], $size, $atts['orientation'], $per_page, $page_index );
